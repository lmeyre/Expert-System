        ...
        if analyze_statement(rule_statement) == true:
            ...
            if current_condition == True:
                for line in rule_deduction
                    if line is not in operators and l # si c'est pas un operators, donc un `A` et qu'il est pas dans les status
                        self.true_facts[line] = True
        #else set deduction to false, or ignore ?

def analyze_statement(rule_statement):
...
def combine_statements(operand, part1, part2):
    if operand == '+':
